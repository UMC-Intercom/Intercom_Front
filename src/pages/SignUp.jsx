...
export default function SignUp() {
  ...
  const isEmailDuplicated = async (email) => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/users/signup/email?email=${(email)}`);
  ...
  const isNicknameDuplicated = async (nickname) => {
    try {
      // axios를 사용하여 서버에 GET 요청을 보냅니다.
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/users/signup/nickname?nickname=${(nickname)}`);
